...
describes what users get. Pick a format with `-o`, or let job58 choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
job58 <command> -o table   # aligned columns for reading
job58 <command> -o jsonl   # one JSON object per line, for piping
job58 <command> -o json    # a single JSON array
job58 <command> -o csv     # spreadsheet friendly
job58 <command> -o tsv     # tab-separated
job58 <command> -o url     # just the URL column
job58 <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
job58 <command> --fields id,title,url
```
...
```bash
job58 <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
job58 <command>            # a table, because this is a terminal
job58 <command> | wc -l    # JSONL, because this is a pipe
```
